...
const CurrencyBlock: React.FunctionComponent<CurrencyBlockProps> = (
  ...
  const getIso = (code: number) =>
    props.iso4217.find(elem => elem.code == code);
  ...
  return (
    <tr className="currency-row" key={props.idx}>
      <td>
        <strong title={code1.currencyName}>{code1.codeName}</strong>
      </td>
      <td>
        <strong title={code2.currencyName}>{code2.codeName}</strong>
      </td>
      <td>{c.rateBuy}</td>
      <td>{c.rateSell}</td>
